...
import { messages } from './PendingWebinarsMessages';
import { useStyles } from './PendingWebinarsStyles';
...
const PendingWebinars = () => {
  const history = useHistory();
  const intl = useIntl();
  const classes = useStyles();
  const [getPendingWebinars] = useApi({
    endpoint: '/webinar/web/list-like',
    method: 'get'
  });
  ...
  useEffect(() => {
    (async () => {
      try {
        const response = await getPendingWebinars();
